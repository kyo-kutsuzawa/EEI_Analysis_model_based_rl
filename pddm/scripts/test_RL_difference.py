...
import pddm.statstics_anlysis.bootstrapped.bootstrap as bs
import pddm.statstics_anlysis.bootstrapped.compare_functions as bs_compare
import pddm.statstics_anlysis.bootstrapped.stats_functions as bs_stats
import matplotlib
...
if args.data_type=="rewards":
    load_data1=np.load(args.job_path1 +"/rollouts_rewardsPerIter.npy")
    load_data2=np.load(args.job_path2 +"/rollouts_rewardsPerIter.npy")
    load_data3 = np.load(args.job_path3 + "/rollouts_rewardsPerIter.npy")
elif args.data_type=="eei":
    load_data1=np.load(args.job_path1 +"/rollouts_EEIPerIter.npy")
    load_data2=np.load(args.job_path2 +"/rollouts_EEIPerIter.npy")
    load_data3 = np.load(args.job_path3 + "/rollouts_EEIPerIter.npy")
elif args.data_type=="ca":
    load_data1=np.load(args.job_path1 +"/rollouts_CAPerIter.npy")
    load_data2=np.load(args.job_path2 +"/rollouts_CAPerIter.npy")
    load_data3 = np.load(args.job_path3 + "/rollouts_CAPerIter.npy")
elif args.data_type=="ene":
    load_data1=np.load(args.job_path1 +"/rollouts_ENEerIter.npy")
    load_data2=np.load(args.job_path2 +"/rollouts_ENEPerIter.npy")
    load_data3 = np.load(args.job_path2 + "/rollouts_ENEPerIter.npy")

range_data=80
mean_load_data1 = load_data1[:range_data,0]
mean_load_data2 = load_data2[:range_data,0]
mean_load_data3 = load_data3[:range_data,0]

print("first data mean is {} .".format(np.mean(mean_load_data1)))
print("second data mean is {} .".format(np.mean(mean_load_data2)))
print("third data mean is {} .".format(np.mean(mean_load_data3)))

std_load_data1 = load_data1[:range_data,1]
std_load_data2 = load_data2[:range_data,1]
std_load_data3 = load_data3[:range_data,1]

iterartion_number = np.arange(0,len(mean_load_data1),1)

if args.data_type=="rewards":
    plt.figure(figsize=(16, 10), frameon=False)
    plt.plot(iterartion_number, mean_load_data1, color="blue", linewidth=3, linestyle='-')
    plt.fill_between(iterartion_number, mean_load_data1 - std_load_data1, mean_load_data1 + std_load_data1,
                     color="blue", alpha=0.2)
    plt.plot(iterartion_number, mean_load_data2, color="red", linewidth=3, linestyle='-')
    plt.fill_between(iterartion_number, mean_load_data2 - std_load_data2, mean_load_data2 + std_load_data2,
                     color="red", alpha=0.2)
    plt.plot(iterartion_number, mean_load_data3, color="green", linewidth=3, linestyle='-')
    plt.fill_between(iterartion_number, mean_load_data3 - std_load_data3, mean_load_data3 + std_load_data3,
                     color="green", alpha=0.2)

    #graph_data = np.concatenate([mean_load_data1, mean_load_data2])
    #plt.ylim(1.2 * min(graph_data), 1.2 * max(graph_data))
    plt.legend()
    plt.xlabel('iteration')
    plt.ylabel('reward')
    plt.title("reward comparison")
    plt.savefig(save_dir + "reward_{}.png".format(args.save_num))
    plt.show()
    ...
    plt.xlabel('iteration')
    plt.ylabel('EEI')
    plt.title("EEI comparison")
    plt.savefig(save_dir + "eei_{}.png".format(args.save_num))
    plt.show()
    ...
    plt.xlabel('iteration')
    plt.ylabel('CA')
    plt.title("CA comparison")
    plt.savefig(save_dir + "CA_{}.png".format(args.save_num))
    plt.show()
    ...
    plt.xlabel('iteration')
    plt.ylabel('ENE')
    plt.title("ENE comparison")
    plt.savefig(save_dir + "ENE_{}.png".format(args.save_num))
    plt.show()
...
if mean_load_data1.mean() < mean_load_data1.mean():
    m_smaller = mean_load_data1.mean()
else:
    m_smaller = mean_load_data2.mean()
...
def welch_test(data1, data2, alpha=0.05, tail=2):
    ...
    if p <= alpha:
        ...
        print("\n\nResults of the Welch's t-test level %02g: there is not enough evidence to prove any order relation between μ1 and μ2." %p ,alpha)
...
def empirical_false_pos_rate(data, alpha=0.05):
    ...
    plt.savefig(save_dir+"test0.png")
    plt.show()
...
def plot_beta(beta, epsilon, sample_size, beta_requirement=0.2):
    ...
    plt.savefig(save_dir+"beta_test.png")
    plt.show()


welch_test(mean_load_data1, mean_load_data2, alpha, tail=2)
bootstrap_test(mean_load_data1, mean_load_data2, alpha)
empirical_false_pos_rate(mean_load_data1, alpha)
beta = compute_beta(epsilon, sample_size, alpha, mean_load_data1, mean_load_data2, beta_requirement=beta_requirement)
